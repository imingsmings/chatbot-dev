...
空行是传输保活，不是业务事件。服务端接受新 ask、建立流后先发送 `\n`，在请求执行期间每次写入完成后约 5 秒发送下一次空行；重放已完成请求仍只返回 `done`。这样模型暂未输出正文、reasoning 摘要或工具事件时，浏览器仍能确认连接存活。前端读取任何 chunk 都重置 15 秒连接空闲计时，但忽略空行，不更新正文、不生成 reasoning、不落库，也不改变协议版本和六种事件。

保活遵守 Web Stream 背压，最多一个保活写入等待完成，不用固定间隔堆积任务。`done` / `error` 前及取消清理时停止计时并等待正在进行的写入；传输写入失败中止上游。保活不重置 `LLM_TIMEOUT_MS`：每次 Provider 请求的有界期限仍覆盖等待响应头和读取响应体；连接中断后 15 秒无任何数据的前端保护也保持不变。
