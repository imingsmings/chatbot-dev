# Chatbot Roadmap 历史阶段记录

本文件保留截至 2026-08-24 的 P0-R21 详细范围、交付结果和验收证据。当前项目状态、下一阶段候选和维护规则以 [当前 Roadmap](roadmap.md) 为准。
...
- R16：消息索引、取消持久化竞态、存储探针、增量摘要和 CDP 门禁加固已完成并验证。
...
- R21：图片附件、DeepSeek Vision、本地持久化、图片上下文预算、分支复制和 schema v2 ZIP 已完成；静态、单元/API、构建、全量 mock 与全量真实 Provider 均通过，Docker 实机新 Volume 恢复按用户要求暂缓。

R11-R21 已完成；R21 的 Docker 实机恢复为明确暂缓项。
...
- 每个会话持久化完整模型配置快照；刷新、切换、分支、导入导出、Docker 重启和 Volume 恢复保持一致。
- 应用 NDJSON v2、reasoning 和工具生命周期。
- DeepSeek `[DONE]` / OpenAI `response.completed` 完整性门禁，异常 EOF 不落库。
- 摘要覆盖边界之后的原始消息窗口，以及可见的覆盖/选择范围统计。
- 摘要只增量处理覆盖边界后的消息；单次模型输入默认限制为 24000 字符，输出最多 1024 tokens。
- 全链路取消、首包/流空闲超时和单会话请求互斥。
- UI 消息保存索引、成功/停止后的服务端详情回拉，以及等待请求清理完成的取消握手。
...
| R16 | 全链路一致性、摘要预算、存储探针和测试门禁加固 | 完成并验证 |
...
| R21 | 图片附件、DeepSeek Vision、本地持久化和 schema v2 ZIP | 完成（Docker 实机恢复暂缓） |
...
1. 自定义 Prompt 模板 CRUD、导入导出和本地持久化。（后续已选择为 R18）
...
- 修复 optimistic UI 消息与已保存消息索引混用后，编辑或重新生成可能指向错误历史的问题。
- 让“停止完成”代表上游已释放且持久化已收口，避免靠固定等待时间猜测服务端状态。
- 让健康检查、摘要成本和 UI 回归结果反映真实实现，而不是只在表面路径上通过。

### 交付结果

- 服务端详情映射为每条 UI 消息保存 `persistedIndex`；编辑/重新生成只接受已保存消息。成功流结束后重新读取会话详情，删除未落库的 optimistic 行并恢复 generation/tool trace 等字段。
- 请求注册表在取消后继续占用 requestId 和 conversationId，直到 ask `finally` 调用 `completeRequest`；取消 API 等待该完成信号并返回 `completed`。前端只在确认完成后拉取详情，500ms 客户端等待上限仅作为网络异常降级。
- `ConversationStore` 增加 `checkHealth` 契约：file store 在实际 conversations 目录写入/读回/删除探针；SQLite 在配置数据库上执行 `BEGIN IMMEDIATE`、写读校验和 `ROLLBACK`。
- 摘要基于 `summary.sourceMessageCount` 只处理新增原始消息，并滚动合并旧摘要；单次 prompt 内容受 `SUMMARY_MAX_INPUT_CHARS` 限制，输出上限固定不超过 1024 tokens。无新增消息不调用模型，停止态正文不进入摘要但会推进覆盖边界。
- UI CDP 由共享 harness 和四个真实场景模块组成，旧 2500 行单体只保留 42 行调度器；upstream cancellation 测试验证 cancel completion、上游关闭、落库与刷新恢复，并在任一场景为 false 时返回非零退出码。

### 验收标准与结果

- 成功、错误、超时和停止后的 UI 均与服务端持久化一致；未保存消息不显示编辑/重新生成入口。
- 手动停止、首 token 前停止、新建切换中断和慢上游中断均确认 cancel completed 与上游连接释放；有正文的手动停止按 `stopped` 恢复，其他中断不写伪消息。
- file/SQLite 健康探针、不可写 503 与恢复、超长摘要分块和增量边界均有自动化断言。
- `pnpm run check`、server 114/114、client 71/71、`pnpm run build`、`pnpm run test:cdp:all-mock`、`pnpm run test:docker` 和生产依赖审计全部通过。
- `pnpm run test:cdp:all-real` 已按隔离批次通过：OpenAI UI/上下文/Markdown/Responses（reasoning、calculator 工具、停止持久化与恢复）全部通过；DeepSeek V4 Flash 的 Off/Low/Medium/High 4/4 通过。已禁用模型未发送真实请求。

### 非目标

- 不改变 NDJSON v2、备份 schema 或 SQLite 表结构。
- 不增加后台任务队列、自动摘要调度、分布式锁或外部健康监控平台。
...
- 解决当前模型、provider、reasoning effort、temperature 和 max tokens 仅存在于页面状态，刷新或切换会话后回到运行时默认值的问题。
- 让不同会话可以稳定绑定不同模型，便于个人使用中的回答对比与长期上下文延续。

### 交付结果

- 会话保存完整且可选的 `modelOptions`；新会话绑定运行时默认值，旧会话首次保存或 ask 前回填，分支继承，清空保留，配置更新不改变 `updatedAt`。
- file store 在同会话 mutation queue 中原子保存；SQLite 幂等增加可空 `model_options` 列并安全降级损坏 JSON；schema v1 以可选字段向后兼容。
- 独立 `PATCH /api/conversations/:id/model-options` 提供 400/404/409/200 语义，并与 ask/摘要共享单会话互斥。
- React 专责 hook 处理 runtime/详情乱序、A/B 恢复、乐观保存、快速点击、失败回滚和切换后的过期响应；保存期间阻止发送、摘要和上下文预览。
- 已禁用、删除或未配置模型回退到当前可用默认值；没有可用 Provider 时保持不可发送，且不向失效模型发请求。
- file/SQLite、导入导出、Markdown 元信息、CDP、Docker 重启和备份到新 Volume 均已验证。
- 详细数据结构、API、迁移、文件范围、测试矩阵和回滚方案见 [会话级模型配置持久化方案](conversation-model-options-plan.md)。
- 最终证据见 [R17 会话级模型配置持久化验收记录](r17-conversation-model-options-2026-08-13.md)。
...
- 让高频个人提示词不再局限于源码中的 6 个固定模板，也不需要在聊天外部反复复制维护。
- 通过本地 JSON 备份在浏览器之间迁移模板，同时保持会话、Provider 和服务端存储边界不变。

### 交付结果

- 保留内置模板只读语义；自定义模板支持新增、编辑、二次确认删除、变量填充和直接写入输入框。
- `{变量名}` 支持 Unicode 字母、数字、下划线和连字符，保持出现顺序并自动去重；独占一行的变量使用多行输入。
- schema v1 数据保存在 `chatbot-custom-prompt-templates` localStorage key；损坏数据明确报错，不作为合法模板加载。
- 导入校验文件版本、数量、ID、名称、内容和变量边界；相同模板跳过，ID 冲突生成新 ID，不静默覆盖本地内容。
- JSON 导出只包含自定义模板和导出时间；模板数据不进入会话备份、file/SQLite、Docker Volume 或模型请求。
- 自动化覆盖纯数据契约、hook、组件和独立 CDP 场景；全量 mock 15/15、真实 DeepSeek 模型配置 8/8 及 OpenAI/DeepSeek 真实 UI 链路均通过。
- 真实回归等待窗口会读取服务端 `LLM_TIMEOUT_MS`，并额外保留 15 秒供 UI 接收最终错误，避免测试脚本早于服务端超时退出。
- 最终证据见 [R18 自定义 Prompt 模板验收记录](r18-custom-prompt-templates-2026-08-13.md)。

### 非目标

- 不增加服务端模板 API、跨设备自动同步、模板市场、共享权限或团队协作。
- 不执行模板中的代码，不允许模板改变模型配置、工具权限或发送行为。
...
- 减少连续输出时的成段跳字、长 Markdown 卡顿和重复滚动布局工作，提升个人日常使用中的阅读连续性。
- 用固定 mock、浏览器性能指标和行为断言区分网络到达、React 更新、Markdown 重算和自动滚动成本，避免只凭主观观感调参。

### 交付结果

- 保持 Provider SSE、应用 NDJSON v2、流完整性、取消语义和持久化格式不变。
- 首段正文或 reasoning 立即显示；后续相邻同类文本以 40ms 合并，并受 100 个事件组或 16384 字符上限保护。tool、done、error 和手动停止前同步冲刷，transition/unmount 丢弃废弃缓冲。
- `MessageRow` memo 隔离未变化历史行，列表条件从逐行切片扫描改为单次线性派生；200 条历史消息流式期间历史行渲染由 20502 次降为 0。
- 自动滚动使用 ResizeObserver 和单一 rAF；真实用户离开底部后不再被抢回，并显示“滚动到底部”按钮，点击后立即到底并恢复当前流跟随。
- Markdown 流式刷新改为 ≤40KB 80ms、>40KB 160ms；4KB、24KB、80KB 的可见更新 P95 中位数分别由 161.1/261.1/456.3ms 降至 102.0/107.5/176.1ms，80KB 场景 5 次均无 long task。
- 诊断 marks 仅在 Vite dev/test 且显式启用时存在，不进入生产 UI、不持久化、不上传；没有新增运行时依赖。
- 最终通过 `check`、server 122/122、client 96/96、build、16/16 全量 mock、Docker smoke/UI、production audit，以及 DeepSeek 真实 UI/上下文/Markdown、OpenAI Responses 和 DeepSeek Flash/Pro × Off/Low/Medium/High 8/8 真实模型矩阵。真实测试使用随机端口、临时 store/Profile，不生成截图，测试会话与进程均清理。
- 详细边界和回滚见 [流式渲染平滑度优化方案](streaming-rendering-optimization-plan.md)，阶段证据见 [R19 流式渲染与快速到底验收记录](r19-streaming-rendering-2026-08-13.md)。
...
R17 会话配置语义、兼容矩阵和最终门禁见 [R17 会话级模型配置持久化验收记录（2026-08-13）](r17-conversation-model-options-2026-08-13.md)。
...
R21 图片附件、Vision、持久化、Mock/真实全量和截图证据见 [R21 图片附件与 Vision 验收记录（2026-08-24）](r21-multimodal-vision-2026-08-24.md)。
