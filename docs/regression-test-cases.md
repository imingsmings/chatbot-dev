...
以下脚本可作为后续自动化整合的基础。若代码结构变化，允许微调脚本，但用例语义保持不变。

| 覆盖范围 | 当前脚本 |
| --- | --- |
| 停止生成、上游取消、request cancel | `.tmp/scripts/cdp-upstream-abort-test.mjs` |
| 会话 API、持久化、错误 JSON、tool mock | `.tmp/scripts/cdp-p0-api-tool-scenarios.mjs` |
| 基础 UI、复制、重试、滚动、新建中断 | `.tmp/scripts/cdp-ui-scenarios.mjs` |
| 真实接口基础 UI | `.tmp/scripts/cdp-real-scenarios.mjs` |
| 会话上下文、重命名、清空、删除 | `.tmp/scripts/cdp-conversation-context-real.mjs` |
| Markdown fixture 渲染 | `.tmp/scripts/cdp-markdown-rendering.mjs` |
| Markdown 真实接口渲染 | `.tmp/scripts/cdp-markdown-real.mjs` |
| 代码高亮 fixture 渲染 | `.tmp/scripts/cdp-highlight-rendering.mjs` |
