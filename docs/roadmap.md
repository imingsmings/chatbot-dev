...
R11-R14 已完成并验证。R15 必须先有明确的真实使用需求再选定单一范围。
...
1. 编辑上一条用户消息、重新生成回答和必要的会话分支。
2. 自定义 Prompt 模板 CRUD、导入导出和本地持久化。
3. 文件附件与受限文本提取；只有明确资料问答需求后再评估 RAG。
4. 移除语音占位，或在确定高频语音需求后实现真正可用的语音输入。

### 进入条件

- R11-R14 已完成，或有明确理由调整顺序且不破坏其数据与协议设计。
- 候选功能有具体使用场景、数据边界、失败恢复和测试方案。
- 用户明确确认选中的单一范围后，才将 R15 改为“实施中”。
...
7. R11-R14 开始实施前先确认阶段范围；完成后必须更新状态、验收证据和相关架构/测试文档。
...
R11-R14 已完成。R15 候选仍必须先说明对个人使用或学习的直接价值，并由用户明确确认单一实施范围。
