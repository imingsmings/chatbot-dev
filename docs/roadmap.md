...
R11-R16 已完成并验证。后续阶段仍必须从真实个人使用证据中一次选择一个范围。
...
- 是否将其作为 R16 之后的下一个单一范围，仍需用户明确确认；本文档完成不等于实现批准。
...
- 在会话中保存完整且可选的 `modelOptions`；新会话绑定运行时默认值，旧会话按需回填，分支继承，清空保留。
- file store 原子保存 JSON 字段；SQLite 幂等增加可空 `model_options` 列；schema v1 备份以可选字段保持向后兼容。
- 增加独立 model-options PATCH；前端用专责 hook 处理加载恢复、保存等待态、快速点击互斥、失败回滚和会话切换竞态。
- 已禁用或删除模型必须回退到当前可用默认模型，且不得向失效模型发送请求。
- 详细数据结构、API、迁移、文件范围、测试矩阵和回滚方案见 [会话级模型配置持久化方案](conversation-model-options-plan.md)。
- 本节完成只表示方案已记录；开始编码仍需用户明确确认，且不能与流式渲染候选同时默认进入实施。
...
R11-R16 已完成。后续候选仍必须先说明对个人使用或学习的直接价值，并由用户明确确认单一实施范围。
