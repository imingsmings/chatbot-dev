...
test('repository exposes one Bun backend and no Node comparison entrypoints', async () => {
  ...
  assert.equal(scripts['dev:server'], 'bun --no-env-file run --cwd bun-server dev')
  assert.equal(scripts['start:server'], 'bun --no-env-file run --cwd bun-server start')
  // dotenv must load the Argon2 PHC hash literally, without Bun dollar expansion.
  const backend = await readJson('bun-server/package.json')
  assert.equal(backend.scripts.dev, 'bun --no-env-file --watch ./bin/www.ts')
  assert.equal(backend.scripts.start, 'bun --no-env-file ./bin/www.ts')
  assert.match(scripts['start:production'], /bun --no-env-file run/)
